...
<Navbar navClass="relative py-3 bg-primary" navDivClass="mx-auto flex flex-wrap space-x-4 items-center px-4 justify-end" fluid={false} color="gray">
  {#if groups}
    <div>
      <Button><Chevron>Select a group</Chevron></Button>
      <Dropdown >
        {#each groups as group}
          <DropdownItem on:click={() => goto(`/groups/${group.id}`)} key={group.id}>{group.name}</DropdownItem>
        {/each}
      </Dropdown>
    </div>
    {/if}
